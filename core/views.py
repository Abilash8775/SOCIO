...
def index(request):
    try:
        ...
        if request.method == 'POST':
            ...
            if User.objects.filter(username=sc).exists():
                return redirect(f'/profile/{sc}')
            else:
                return render(request, 'pnp.html', {'sc': sc})
...
def like_post(request):
    try:
        ...
        logger.error(f"Error in like_post view: {e}")
        return HttpResponse("Internal Server Error", status=500)
...
def follow(request):
    try:
        ...
        logger.error(f"Error in follow view: {e}")
        return HttpResponse("Internal Server Error", status=500)
...
def signin(request):
    try:
        ...
        logger.error(f"Error in signin view: {e}")
        return HttpResponse("Internal Server Error", status=500)
...
def logout(request):
    try:
        ...
        logger.error(f"Error in logout view: {e}")
        return HttpResponse("Internal Server Error", status=500)
